# -*- coding: utf-8 -*-
"""
Market Decision Dashboard — Plotly Dash (CLEAN)
- Stable sizes on first render
- Risk & sizing (ATR), earnings guard, market regime, beta
- News sentiment with policy-risk penalty
- Multi-tile gauges + summary
Run:
...
def perbar_scores(price: pd.Series, rsi_s: pd.Series, pb_s: pd.Series,
    ...
    """Compute per‑bar buy/sell scores with safe caps and NaN guards."""
    ...
    for i in range(len(price)):
        ...
        if pd.notna(vr):
            if vr > 1:
                bs += min(15.0, (vr - 1.0) * 15.0)  # +15 at 2x volume
...
def fetch_macro() -> Dict[str, pd.DataFrame]:
    ...
    try:
        out["btc"] = yf.download("BTC-USD", period="7d", interval="1h", auto_adjust=False, progress=False)
        out["eth"] = yf.download("ETH-USD", period="7d", interval="1h", auto_adjust=False, progress=False)
    except Exception:
...
def compute_probability_and_reasons(
    ...
    try:
        ...
        pass
    earnings_penalty = -0.25 if (days_to_earn is not None and days_to_earn <= 3) else 0.0
    ...
    if news_edge > 0.05:
        bull.append("News sentiment positive (recent)")
    elif news_edge < -0.05:
        bear.append("News sentiment negative (recent)")
    if vix_pct > 0.7:
        bear.append("VIX elevated vs 6m (volatility risk)")
    if t in {"HOOD", "COIN", "MSTR", "MARA", "RIOT", "PYPL", "SQ"}:
        if crypto_24h > 0:
            bull.append("Crypto 24h up (supportive)")
        elif crypto_24h < 0:
            bear.append("Crypto 24h down (headwind)")
    if policy_hit:
        bear.append("Policy/tariff/regulatory headline — caution")
    ...
    try:
        if not spy_series.empty and len(p) >= 60:
            ...
            y = r_t.loc[j].values
            x = r_spy.loc[j].values
            if len(j) > 20:
                A = np.column_stack((x, np.ones_like(x)))
                beta_val = float(np.linalg.lstsq(A, y, rcond=None)[0][0])
    except Exception:
        beta_val = None

    exp_R = float(round((prob_up * rr_target) - ((1.0 - prob_up) * 1.0), 3))
...
def probability_gauge(title: str, pct: float, mini: bool = False) -> go.Figure:
    fig = go.Figure(go.Indicator(
        ...
        value=pct,
        title={"text": title},
        number={"suffix": "%", "font": {"size": 26 if mini else 34}},
        gauge={
            "axis": {"range": [0, 100]},
            "bar": {"thickness": 0.25},
            "steps": [
...
def update_detail(ticker, _n, capital, risk_pct, rr_target, atr_mult):
    ...
    _prob, out = compute_probability_and_reasons(
        ticker,
    ...
    bullets += [
        html.Li(f"ATR(14): {atr_now:.2f}"),
        html.Li(f"Stop: {float(atr_mult or 1.5)}×ATR → ${stop_dist:.2f}"),
        html.Li(f"Take‑profit: {float(rr_target or 1.5):.1f}R → ${take_profit:.2f}"),
        html.Li(f"Risk per trade: ${risk_amt:.2f}"),
        html.Li(f"Suggested size: {shares} sh @ ${px:.2f}"),
        html.Li(f"Expected R: {exp_R:+.3f}"),
    ]
...
def update_tiles(tickers, _n, capital, risk_pct, rr_target, atr_mult):
    if not tickers:
